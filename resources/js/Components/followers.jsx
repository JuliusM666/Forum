...
export default function Followers({ theme, isFollowing }) {
    ...
    return (
        <Link as="button" type="button" disabled={auth.user == null} href="/follow" method="post" preserveState preserveScroll data={{ theme_id: theme.id }}
            ...
            <div className="rounded-full bg-slate-100  px-4 py-1">
                <h1 className="text-sm text-slate-700 ">{theme.followers_count}</h1>
            </div>
        </Link>
    )
}