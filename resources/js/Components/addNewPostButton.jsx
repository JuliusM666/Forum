...
export default function AddNewPostButton({ topics, defaultID }) {
    ...
    function submit(e) {
        ...
        post('/post', {
            preserveScroll: true,
            onSuccess: () => { clearForm(), setIsComponentVisible(false) }
    ...
    return (
        <>
            <div className='flex md:justify-end max-sm:justify-center max-sm:m-1'>
                <button onClick={() => setIsComponentVisible(!isComponentVisible)} className='bg-blue-300 rounded-3xl hover:bg-opacity-80  py-2 px-4 text-center md:max-w-fit max-sm:w-full  font-semibold max-h-fit text-slate-200'><i className='fa fa-plus-circle mr-2' />New Post</button>
            </div>
            {isComponentVisible &&
                <div ref={ref}>
                    <AddModal name={"Add new post: topic1"} handleCloseClick={() => setIsComponentVisible(false)}>
