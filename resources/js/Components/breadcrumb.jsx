                    ...
                    {Object.keys(breadcrumbs).map(function (keyName, keyIndex) {
                        ...
                        return (
                            <li key={keyIndex} className="text-xs text-slate-400 font-semibold mx-2 ">
                                <Link preserveState preserveScroll className=" hover:bg-white hover:opacity-25" href={breadcrumbs[keyName].route}>{breadcrumbs[keyName].name} <i className="fa fa-solid fa-angle-right" /> </Link>
                            </li>
                        )
                    })}
