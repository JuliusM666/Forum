...
export default function UserSettingsModal({isVisible,isShowSettings,setIsShowSettings,user}){
    ...
    function submit(e) {
        e.preventDefault()
        post(route('user.update',user), {preserveScroll: true,
            onSuccess: () =>{setIsShowSettings(false)}})
      }
      const { flash } = usePage().props
    return(
        <div style={{visibility: isVisible? "visible":"hidden"}}>
        <Modal> <Card name="User settings" shadow='' ButtonComponent={<CloseButton handleOnClick={()=>setIsShowSettings(false)}/>}>
            <div className='bg-slate-100 rounded-b-lg h-full shadow-inherit'>
                    ...
                    <form onSubmit={submit} id="settings_form" >
                        <div className="grid grid-cols-1 gap-3 p-6">
                            ...
                            <input onChange={e=>setData('user_img', e.target.files[0])}  className="rounded-lg bg-slate-200 text-slate-600 text-md border-slate-700 border
                            ...
                            <input onChange={e=>setData('banner_img', e.target.files[0])} className="rounded-lg bg-slate-200 text-slate-600 text-md border-slate-700 border
